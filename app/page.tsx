    ...
    <div className="min-h-screen bg-white text-black font-sans selection:bg-red-100 overflow-x-hidden">
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-md border-b border-gray-100 transition-transform duration-300 ${isNavVisible ? 'translate-y-0' : '-translate-y-full'}`}>
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-center relative">
          ...
          <button className="md:hidden absolute right-6 p-2 text-gray-600">
                    ...
                    {[
                      ...
                      >
                        <div className="text-red-500 mb-2 flex justify-center">
                          <svg className="w-6 h-6" fill={link.isWhatsApp || link.label === "LinkedIn" ? "currentColor" : "none"} stroke={link.isWhatsApp || link.label === "LinkedIn" ? "none" : "currentColor"} viewBox="0 0 24 24" strokeWidth={2}>
                            {link.icon}
                          </svg>
                        </div>
